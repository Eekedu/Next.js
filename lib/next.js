...
function createCanvas(width2, height2, cursorHide){
  if (!canvas){
    ...
    canvas.addEventListener("mousemove", function(event){
      ...
      mouseX = (canvasIsRotate)?event.clientY:event.clientX;
      mouseY = (canvasIsRotate)?event.clientX:event.clientY;
    });
    canvas.addEventListener("touchmove", function(event){
      if (typeof mouseMove === "function") { mouseMove(); }
      mouseX = (canvasIsRotate)?event.touches[0].clientY:event.touches[0].clientX;
      mouseY = (canvasIsRotate)?event.touches[0].clientX:event.touches[0].clientY;
    });
    ...
    window.addEventListener("keyup", function(event){
      if (typeof keyTyped === "function") { keyTyped(event.key); }
      var small = event.key.toLowerCase();
